...
function MatchesList() {
  const { data, loading, error, subscribeToMore } = useQuery(ALL_MATCHES_QUERY, { fetchPolicy: 'network-only' });
  ...
  useEffect(() => {
    const unsubscribe = subscribeToMore({
      ...
      updateQuery: (prev, { subscriptionData }) => {
        ...
        if (prev.allMatches.find((m: any) => m.id === newMatch.id)) {
          return prev;
        }
  ...
  if (loading) return <div className='text-center py-20 animate-pulse text-sage-green'>Finding matches...</div>;
  ...
  return (
    <div className='space-y-6'>
      {matches.map((match: any) => (
        <div key={match.id} className='p-8 bg-white rounded-3xl border border-border shadow-nurture-lg relative overflow-hidden group'>
          ...
          <div className='flex flex-col gap-4'>
            <div>
              ...
              <p className='text-muted-foreground uppercase tracking-widest text-xs font-bold mt-1'>{match.name.gender} Gender</p>
            </div>

            <div className='flex flex-wrap items-center gap-2 mt-4'>
              <span className='text-sm font-bold text-charcoal/40 mr-2'>LIKED BY:</span>
              {match.likedBy.map((user: any) => (
                <span
                  key={user.username}
