    ...
    <div className='min-h-screen flex flex-col items-center justify-center p-6'>
      <div className='w-full max-w-lg'>
        ...
        <div className='bg-white/80 backdrop-blur-sm rounded-3xl p-8 shadow-nurture border border-border/50'>
          ...
          {step === 2 && <PreferencesStep preferences={preferences} onPreferencesChange={setPreferences} />}
          {step === 3 && (
            <ConfirmStep users={users} preferences={preferences} onComplete={handleComplete} loading={loading} error={error} />
          )}
        </div>
        ...
        <div className='flex justify-between mt-6'>
          <button
            ...
            className={`flex items-center gap-2 px-6 py-3 rounded-xl font-bold transition-all ${
              step === 1 ? 'text-muted-foreground cursor-not-allowed' : 'text-charcoal hover:bg-calm-ivory'
            }`}>
            <ChevronLeft size={20} />
            Back
          </button>
