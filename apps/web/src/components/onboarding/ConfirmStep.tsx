...
export function ConfirmStep({ users, preferences, onComplete, loading, error }: ConfirmStepProps) {
  const countryNames = preferences.countryOrigins.map((code) => SUPPORTED_COUNTRIES.find((c) => c.code === code)?.name || code).join(', ');

  return (
    <div className='space-y-6'>
      <div className='text-center'>
        <h2 className='text-2xl font-heading text-primary mb-2'>Ready to Start!</h2>
        <p className='text-muted-foreground'>Review your setup and start finding the perfect name.</p>
      </div>
        ...
        <div className='flex items-start gap-4 p-4 bg-calm-ivory rounded-2xl'>
          ...
          <div>
            ...
            <p className='text-muted-foreground'>
              {preferences.genderPreference === 'both' ? 'All genders' : `${preferences.genderPreference} names only`}
              <br />
      ...
      <button
        ...
        className={`w-full py-4 rounded-2xl font-heading font-bold text-lg flex items-center justify-center gap-3 transition-all ${
          loading ? 'bg-primary/70 text-white cursor-wait' : 'bg-primary text-white hover:bg-primary/90 shadow-nurture active:scale-[0.98]'
        }`}>
        {loading ? (
          <>
      ...
      <p className='text-center text-sm text-muted-foreground'>This will seed the name database and prepare your experience.</p>
    </div>
  );
}
