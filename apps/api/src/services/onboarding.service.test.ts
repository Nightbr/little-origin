...
import { users, appSettings, preferences, names } from '@little-origin/core';
...
describe('OnboardingService Integration Tests', () => {
  ...
  it('should block adding users after onboarding is complete', async () => {
    ...
    await expect(onboardingService.addUser('late_user', 'password123')).rejects.toThrow('Onboarding already completed');
  });
});
